...
class TrainingConfig:
    ...
    output_dir = "pose"  # the model name locally and on the HF Hub
...
print("Mask type:", type(dataset[0]["condition_image"]))
...
print("POSKUS Z DODAJANJEM POZE")
...
maska = dataset[0]["condition_image"].unsqueeze(0)
...
def train_loop(config, model, noise_scheduler, optimizer, train_dataloader, lr_scheduler):
    ...
    for epoch in range(config.num_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            input = torch.cat((noisy_images, pose), dim=1)
