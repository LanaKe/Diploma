...
class DDPMPipeline(DiffusionPipeline):
    ...
    def __call__(
        ...
        condition = condition.to(self.device, dtype=self.unet.dtype).unsqueeze(0)
        print(image.shape, condition.shape)
        print(type(image), type(condition))
        ...
        print("to sta image in shape:", type(image), image.shape)
        image = (image / 2 + 0.5).clamp(0, 1)
        image = image.cpu().permute(0, 2, 3, 1).numpy()
        if output_type == "pil":
            print("output type je pil")
